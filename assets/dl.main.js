...
function getOSAsset(assets, os) {
  // 优先顺序: win->exe, mac->dmg/pkg/zip, linux->AppImage/tar.gz
  if (os === 'windows') {
    return assets.find(a => /\.exe$/i.test(a.name)) || assets.find(a => /\.zip$/i.test(a.name));
  }
  if (os === 'macos') {
    return assets.find(a => /\.dmg$/i.test(a.name)) || assets.find(a => /\.pkg$/i.test(a.name)) || assets.find(a => /\.zip$/i.test(a.name));
  }
  if (os === 'linux') {
...
function renderReleaseList(rel, os) {
  const list = document.getElementById('release-list');
  list.innerHTML = `
        <div class="border-b pb-2 mb-2">
          <div class="font-bold text-lg">${rel.name || rel.tag_name} <span class="text-xs text-gray-400">(${formatDate(
    rel.published_at
  )})</span></div>
          <div class="flex flex-col gap-2 mt-2">
            ${rel.assets
              ...
              <a href="${ghproxy}${
                ...
                title="${a.name}">
                <span class="mr-1">${assetIcon(a.name)}</span>${a.name}
                <span class="ml-2 text-gray-400">${formatSize(a.size)}</span>
              </a>
            `
              )
              .join('')}
  ...
  bestDiv.innerHTML = `
        <div class="highlight-download rounded-lg p-6 flex flex-col items-center max-w-md w-full">
          ...
          <a href="${ghproxy}${asset.browser_download_url}" target="_blank"
            ...
            <span class="mr-2 text-2xl">${assetIcon(asset.name)}</span>${asset.name}
          </a>
          <div class="text-gray-500 text-sm">${translate('Version')}: ${latest.name || latest.tag_name} | ${translate(
    'Release Date'
  )}: ${formatDate(latest.published_at)} | ${translate('Size')}：${formatSize(asset.size)}</div>
        </div>
      `;
}
...
(async function () {
  ...
  const r = await fetch(`${lang === 'zh' ? '' : '../'}data/version.json?_=${Date.now()}`).then(d => d.json());
  renderBestDownload(r, os);
  renderReleaseList(r, os);
})();
